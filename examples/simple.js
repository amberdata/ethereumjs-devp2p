...
async function main() {
  ...
  try {
    const { rows } = await client.query(`select hostname from node2 where ("lastAsked" is null or "lastAsked" < now() - interval '60' minute) order by timestamp desc limit 2000`, [])
    hostnames = []
    for (let row of rows) {
